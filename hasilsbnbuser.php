	...
	<div style="background-color: #efefef">
		<div class="container">
			<div class="section-card" style="padding: 20px 0px">
				...
				<center>
					<h4 class="header" style="margin-left: 24px; margin-bottom: 0px; margin-top: 24px; color: #635c73;">HASIL REKOMENDASI SABUN WAJAH</h4>
				</center>
				<ul>
					<li>
						<div class="row">
							<div class="card">
								<div class="card-content">
									<h5 style="margin-bottom: 16px; margin-top: -6px;">Matrik Sabun Wajah</h5>
									<table class="responsive-table">
										...
										<tbody>
											<?php
											$query=mysqli_query($selectdb,"SELECT * FROM tb_berminyak");
											$no=1;
											while ($data_sabun=mysqli_fetch_array($query)) {
												$Matrik[$no-1]=array($data_sabun['merek_angka'],$data_sabun['kesesuaian_angka'],$data_sabun['harga_angka'],$data_sabun['kualitas_angka'],$data_sabun['umur_angka'] );
												?>
												<tr>
													<td><center><?php echo "A",$no ?></center></td>
													<td><center><?php echo $data_sabun['merek_angka'] ?></center></td>
													<td><center><?php echo $data_sabun['kesesuaian_angka'] ?></center></td>
													<td><center><?php echo $data_sabun['harga_angka'] ?></center></td>
													<td><center><?php echo $data_sabun['kualitas_angka'] ?></center></td>
													<td><center><?php echo $data_sabun['umur_angka'] ?></center></td>
												</tr>
												<?php
												$no++;
											}
				...
				<ul>
									...
									<table class="responsive-table">
										...
										<tbody>
											...
											while ($data_sabun=mysqli_fetch_array($query)) {
												...
												$MatrikNormalisasi[$no-1]=array($data_sabun['merek_angka']/$pembagiNM[0],
													$data_sabun['kesesuaian_angka']/$pembagiNM[1],
													$data_sabun['harga_angka']/$pembagiNM[2],
													$data_sabun['kualitas_angka']/$pembagiNM[3],
													$data_sabun['umur_angka']/$pembagiNM[4]);

													?>
													<tr>
														<td><center><?php echo "A",$no ?></center></td>
														<td><center><?php echo round($data_sabun['merek_angka']/$pembagiNM[0],6)?></center></td>
														<td><center><?php echo round($data_sabun['kesesuaian_angka']/$pembagiNM[1],6) ?></center></td>
														<td><center><?php echo round($data_sabun['harga_angka']/$pembagiNM[2],6) ?></center></td>
														<td><center><?php echo round($data_sabun['kualitas_angka']/$pembagiNM[3],6) ?></center></td>
														<td><center><?php echo round($data_sabun['umur_angka']/$pembagiNM[4],6) ?></center></td>
													</tr>
													<?php
													$no++;
												}
										...
										<table class="responsive-table">
											<thead>
												<tr>
													...
													<th><center>Value Kriteria Kesesuaian</center></th>
													<th><center>Value Kriteria Harga</center></th>
													<th><center>Value Kriteria Kualitas</center></th>
													<th><center>Value Kriteria Usia</center></th>
												</tr>
											</thead>
											<tbody>
												<!--count($W)-->
										...
										<div class="card-content">
											...
											<table class="responsive-table">
												...
												<tbody>
													...
													<tr>
														<?php  
															$idealpositif=array(max($NormalisasiBobotTrans[0]),max($NormalisasiBobotTrans[1]),min($NormalisasiBobotTrans[2]),max($NormalisasiBobotTrans[3]),max($NormalisasiBobotTrans[4]));
                                                            ?>
                                                        ...
                                                        <tr>
                                                            <?php  
                                                            $idealnegatif=array(min($NormalisasiBobotTrans[0]),min($NormalisasiBobotTrans[1]),max($NormalisasiBobotTrans[2]),min($NormalisasiBobotTrans[3]),min($NormalisasiBobotTrans[4]));
                                                            ?>
                                                            <td><center><?php echo "Y-" ?> </center></td>
											...
											<table class="responsive-table" >
												...
												<thead style="border-top: 1px solid #d0d0d0;">
													<tr>
														...
														<th></th>
														<th><center>Alternatif Sabun Wajah terpilih</center></th>
													</tr>
												</thead>
												<tbody>
													<tr>
														...
														for ($i=0; $i < count($nilaiV); $i++) { 
															if ($nilaiV[$i] == $testmax) {
																$query=mysqli_query($selectdb,"SELECT * FROM tb_berminyak where id_sbnb = $i+1");
																?>
																<td><center><?php echo "V".($i+1); ?></center></td>
																<td><center><?php echo $nilaiV[$i]; ?></center></td>
																<?php while ($user=mysqli_fetch_array($query)) { ?>
																<td><center><?php echo $user['merek_sabun']; ?></center></td>
																<?php
															}
														}
													} ?>
					...
					<div class="row center" \>
						<a href="rekomendasi_sbnbuser.php" id="download-button" class="waves-effect waves-light btn" style="margin-top: 0px">Hitung Rekomendasi Ulang</a>
					</div>
				</div>
			</div>
		</div>
